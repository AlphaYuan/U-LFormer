...
def test_save(device,addre,filep_valid_id,save_dir,exp=None,fov=None,model_cp=True,addre_stage2=None,addre_state=None,addre_cp=None,
    ...
    for i in range(20):
        ...
        results_by_cpnum[i] = []
    cpnum_list = []
    preds1 = np.concatenate(preds, axis=0)
    pad1 = np.concatenate(pad, axis=0)
    trues1 = np.concatenate(trues, axis=0)
    for idx in range(preds1.shape[0]):
        # traj_len = torch.sum(pad1[idx, :]).numpy()
        labs = trues1[idx]
        cp_idx = np.argwhere((labs[:-1, :] != labs[1:, :]).sum(1) != 0).flatten()+1
        cp_idx_new = merge_cp(cp_idx)
        cpnum_list.append(len(cp_idx_new))
        data_by_cpnum[len(cp_idx_new)].append(idx)
    for key, val in data_by_cpnum.items():
        print(key, len(val))
    ...
    ensemble_pred_cp_all, ensemble_true_cp_all, paired_alpha_all, paired_D_all, paired_s_all = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list)
    # for i in range(20):
    ...
    for kk in range(len(preds)):
        ...
        for i in range(total_num):
            ...
            cp_idx_new = merge_cp(trues_cp)
            cp_num = len(cp_idx_new)
            ...
            ensemble_pred_cp_all[cp_num].append(preds_cp[:-1])
            ensemble_true_cp_all[cp_num].append(trues_cp[:-1])
            pair_a, pair_d, pair_s = segment_property_errors(trues_cp, trues_alpha, trues_D, trues_s, 
                                                            preds_cp, preds_alpha, preds_D, preds_s,
                                                            return_pairs = True)
            try:
                paired_alpha_all[cp_num] = np.vstack((paired_alpha_all[cp_num], pair_a))
                paired_D_all[cp_num] = np.vstack((paired_D_all[cp_num], pair_d))
                paired_s_all[cp_num] = np.vstack((paired_s_all[cp_num], pair_s))        
            except:
                paired_alpha_all[cp_num] = pair_a
                paired_D_all[cp_num] = pair_d
                paired_s_all[cp_num] = pair_s

            cnt_idx = cnt_idx + 1

    print(len(ensemble_pred_cp_all[0]))
    print(paired_alpha_all[0].shape)
    for cp_num in ensemble_pred_cp_all.keys():
        if ensemble_pred_cp_all[cp_num] == []:
            continue
        paired_alpha1 = paired_alpha_all[cp_num]
        paired_D1 = paired_D_all[cp_num]
        paired_s1 = paired_s_all[cp_num]
        ensemble_pred_cp1 = ensemble_pred_cp_all[cp_num]
        ensemble_true_cp1 = ensemble_true_cp_all[cp_num]

        # checking for nans and problems in predictions
        wrong_alphas = np.argwhere(np.isnan(paired_alpha1[:, 1]) | (paired_alpha1[:, 1] > 2) | (paired_alpha1[:, 1] < 0)).flatten()
        paired_alpha1[wrong_alphas, 1] = paired_alpha1[wrong_alphas, 0] + threshold_error_alpha

        wrong_ds = np.argwhere(np.isnan(paired_D1[:, 1])).flatten()
        paired_D1 = np.abs(paired_D1)
        paired_D1[wrong_ds, 1] = paired_D1[wrong_ds, 0] + threshold_error_D
        
        wrong_s = np.argwhere((paired_s1[:, 1] > 4) | (paired_s1[:, 1]<0))
        paired_s1[wrong_s, 1] = threshold_error_s    
        
        # Changepoints
        rmse_CP, JI = ensemble_changepoint_error(ensemble_true_cp1, ensemble_pred_cp1, threshold = threshold_cp)
            
        # Segment properties
        error_alpha = metric_anomalous_exponent(paired_alpha1[:,0], paired_alpha1[:,1])
        error_D = metric_diffusion_coefficient(paired_D1[:,0], paired_D1[:,1])
        error_s = metric_diffusive_state(paired_s1[:,0], paired_s1[:,1])

        results = {'cp_num': cp_num,
                'rmse_CP': rmse_CP,
                'JI': JI,
                'alpha': error_alpha,
                'D': error_D,
                'state': error_s,}
        print(results)
        results_by_cpnum[cp_num] = results

    with open(os.path.join(save_dir, 'eval_res_bycpnum.txt'), 'a') as f:
        f.write("\n{}, {}, {}, {}, {}\n".format(str(time.asctime(time.localtime())), str(addre), str(filep_valid_id), str(model_cp), str(addre_stage2)))
        f.write(str(results_by_cpnum))
...
if __name__ == '__main__':
    ...
    length='20-200-OurNoFlipNew-pow10-all'
    addre_list = [
        ...
        '/data1/jiangy/andi_tcu/code/andi_2/output/_2024-11-12_15-20-50_4Yp/' # for TAMSD
                ]
    notes_map = ['LSTM', 'CNN-Trans', 'WaveNet', 'Ours (Loss New)', 'No Loss New'] # Ours without flip, with loss
    # notes_map = ['Ours (Loss New)', 'No Loss New']
    valid_file_list = [
    ...
    # valid_file_list = ['/data1/jiangy/andi_tcu/code/andi_2/datasets/andi_set/1213_multi_state/test/test0_Nall_30000_T20_200_K-3_3.csv',
    ...
    # with open("/data1/jiangy/andi_tcu/code/andi_2/test_results/20-200_data_output/data_list_multi_nosigma.pkl", "wb") as f:
    #     pickle.dump(results_data, f)
    


    df = pd.DataFrame(results_all)
    df.to_excel(f'test_results/results_{length}_bycpnum.xlsx', index=False)
    with open(f'test_results/results_{length}_bycpnum.json', 'w') as f:
