...
def load_model(model, model_path, optimizer=None, resume=False, change_output=False,
    ...
    if change_output:
        for key, val in checkpoint['state_dict'].items():
            if key.startswith('output_layer'):
                state_dict.pop(key)
    model.load_state_dict(state_dict, strict=False) #  可通过在load_state_dict()函数中将strict参数设置为False来忽略非匹配键的参数
    ...
    if optimizer is not None and resume:
        if 'optimizer' in checkpoint:
            ...
            print('No optimizer parameters in checkpoint.')   #best没有保存optimizer last有
